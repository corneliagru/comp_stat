...
dens <- function(x, delta) {
  if (x < 0) return(0 * delta)  #so dimension fit
...
bs_non_param <- function(x, k = 2, initial = 1, sta = seq(min(x):max(x)),
  ...
  for (i in 1:B) {
    ...
    temp <- EM(x_bs, k = k, initial = initial, sta = sta,
...
MCMC_iid <- function(res, lag = 35, burn_in = 100) {
  n <- nrow(res)
  keep <- res[burn_in:n, ][seq(1, n-burn_in , by = lag)]
  keep
}



bs_param <- function(B = 100, delta, p, theta_0 = 1, n = 15000, lag = 40, burn_in = 100){
  k <- length(delta)
